...
/**
 * 获取子分类列表
 * @param {object} args 参数对象，包含分类ID
 * @returns {Promise<string>} JSON字符串的子分类列表
 */
async function getSubclassList(args) {
    ...
    try {
        const url = `${appConfig.webSite}/subclass/${args.type_id}`;
        const res = await req(url);
        const $ = cheerio.load(res.data);

        const subclasses = $(".subclass-item");
        subclasses.each((_, el) => {
            backData.data.push({
                subclass_id: $(el).data("id"),
                subclass_name: $(el).text().trim()
            });
        });
    } catch (error) {
        backData.error = `获取子分类列表失败：${error.message}`;
    }
    return JSON.stringify(backData);
}
